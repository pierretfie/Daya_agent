...
"""
    ...
    def handle_security_tool_query(self, tool_name):
        """Handle queries about security tools responsibly"""
        tool_info = self.tool_manager.get_security_tool_info(tool_name)
        
        # Generate response based on available information
        response = {
            "overview": self._generate_tool_overview(tool_info),
            "technical_info": self._generate_technical_info(tool_info),
            "ethical_guidelines": tool_info.get('ethical_notice', 
                "This tool should only be used for legitimate security testing with proper authorization."),
            "learning_resources": self._generate_learning_resources(tool_info)
        }
        
        return self._format_security_response(response)

    def _generate_tool_overview(self, tool_info):
        """Generate tool overview from available information"""
        overview = ["[SECURITY NOTICE]",
                   "This information is provided for educational purposes and legitimate security testing only.\n"]
        
        if tool_info.get('purpose'):
            overview.append(f"Purpose: {tool_info['purpose']}")
        
        if tool_info.get('legitimate_uses'):
            overview.append("\nLegitimate Uses:")
            overview.extend([f"- {use}" for use in tool_info['legitimate_uses']])
        
        return "\n".join(overview)

    def _generate_technical_info(self, tool_info):
        """Generate technical information from available info"""
        tech_info = []
        
        if tool_info.get('syntax'):
            tech_info.extend(["\nBasic Syntax:", tool_info['syntax']])
        
        if tool_info.get('parameters'):
            tech_info.extend(["\nKey Parameters:"])
            tech_info.extend([f"- {param}: {desc}" 
                             for param, desc in tool_info['parameters'].items()])
        
        if tool_info.get('examples'):
            tech_info.extend(["\nExample Usage (Legitimate Scenarios):"])
            tech_info.extend([f"# {name}:\n{cmd}" 
                             for name, cmd in tool_info['examples'].items()])
        
        return "\n".join(tech_info)

    def _generate_learning_resources(self, tool_info):
